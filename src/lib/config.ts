const config = {
  ...
  // Per-ping pong deadline. If a pong (or any inbound frame) does not arrive
  // within this window after we send ws.ping(), the connection is treated as
  // dead and terminated immediately so incoming calls don't keep routing to a
  // ghost socket. Worst-case detection becomes pingInterval + pongTimeout.
  pongTimeout: Number(process.env.PONG_TIMEOUT) || (5 * 1000),
  redis: {
  ...
  // setKeepAlive(true, N) start delay (ms) on the raw TCP socket. Backstops
  // the application-level ping when it stalls for any reason.
  tcpKeepAliveDelay: Number(process.env.TCP_KEEPALIVE_DELAY) || (15 * 1000),
  web: {
